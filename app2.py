...
@app.route('/')
def index():
    return render_template('index3.html', chat_history=chat_history)
...
def chat():
    ...
    def complaints(complaints_query=question):
        ...
        complaints_url = "https://wemabank/complaints/forms"

        # return json.dumps({"Lodge your complaint using this link": complaints_url, "complaints_query": complaints_query})
        return json.dumps({"Lodge your complaint using this link": complaints_url, })
    ...
    # def Check_balance(email=token=question):
    #     # Connect to the SQLite database
    #     conn = sqlite3.connect('bank_database.db')
    #     cursor = conn.cursor()
